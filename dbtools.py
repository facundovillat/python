...
def _load_config_defaults():
    ...
    if os.path.exists(path):
        try:
            ...
            pass
    if 'origen' not in cfg:
        cfg['origen'] = {
            'host': 'localhost',
            'user': 'root',
            'password': '',
            'database': 'ejemploorigen'
        }
    if 'destino' not in cfg:
        cfg['destino'] = {
            'host': 'localhost',
            'user': 'root',
            'password': '',
            'database': 'ejemplodestino'
        }
    return cfg

def _save_config(host_or, user_or, pass_or, db_or, host_dest, user_dest, pass_dest, db_dest):
    cfg = configparser.ConfigParser()
    cfg['origen'] = {
        'host': host_or,
        'user': user_or,
...
class App:
    def __init__(self, root):
        ...
        self._cfg = _load_config_defaults()
        ...
        self.host_or = tk.Entry(frame_conf); self.host_or.grid(row=0, column=1); self.host_or.insert(0, self._cfg['origen'].get('host','localhost'))
        self.user_or = tk.Entry(frame_conf); self.user_or.grid(row=1, column=1); self.user_or.insert(0, self._cfg['origen'].get('user','root'))
        self.pass_or = tk.Entry(frame_conf, show="*"); self.pass_or.grid(row=2, column=1); self.pass_or.insert(0, self._cfg['origen'].get('password',''))
        self.db_or   = tk.Entry(frame_conf); self.db_or.grid(row=3, column=1); self.db_or.insert(0, self._cfg['origen'].get('database','ejemploorigen'))

        self.host_dest = tk.Entry(frame_conf); self.host_dest.grid(row=0, column=3); self.host_dest.insert(0, self._cfg['destino'].get('host','localhost'))
        self.user_dest = tk.Entry(frame_conf); self.user_dest.grid(row=1, column=3); self.user_dest.insert(0, self._cfg['destino'].get('user','root'))
        self.pass_dest = tk.Entry(frame_conf, show="*"); self.pass_dest.grid(row=2, column=3); self.pass_dest.insert(0, self._cfg['destino'].get('password',''))
        self.db_dest   = tk.Entry(frame_conf); self.db_dest.grid(row=3, column=3); self.db_dest.insert(0, self._cfg['destino'].get('database','ejemplodestino'))

        tk.Button(root, text="Conectar", command=self.conectar).pack(pady=5)
        tk.Button(root, text="Salir", command=self.on_close).pack(pady=5)
    ...
    def conectar(self):
        _save_config(
            self.host_or.get(), self.user_or.get(), self.pass_or.get(), self.db_or.get(),
            self.host_dest.get(), self.user_dest.get(), self.pass_dest.get(), self.db_dest.get()
        )
